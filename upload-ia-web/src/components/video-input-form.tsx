...
export function VideoInputForm(props: VideoInputFormProps) {
    ...
    const [codigoStatus, setCodigoStatus] = useState<Number | null>(null)
    ...
    async function handleUploadVideo(event: FormEvent<HTMLFormElement>) {
        ...
        await api.post(`/videos/${videoId}/transcription`,  {
            ...
            setStatus('error')
            setCodigoStatus(err.response.status)
  ...
  text-muted-foreground hover:bg-primary/5">
            ...
            <Button data-success={status === 'success' && codigoStatus === 200} data-erro={status === 'error' && codigoStatus != 200} disabled={status != 'waiting'} type="submit" className="w-full">
               {status === 'waiting' ? (
                <>
                 Carregar video
                <Upload className="w-4 h-4 ml-2 data=[success=true]:bg-emerald-400 data=[erro=true]:bg-red-400" />
                </>
